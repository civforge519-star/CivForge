...
The map renderer uses a **cached mapCanvas approach** for fast, efficient rendering:

1. **Tile Normalization**: On snapshot receipt, tiles (array of objects with `x`, `y`, `biome`, `river`, etc.) are normalized into dense grids:
   - `biomeGrid`: Uint8Array of biome IDs (0-8)
   - `riverGrid`: Uint8Array (0/1 for river presence)
   - `contestedGrid`: Uint8Array (0/1 for contested tiles)

2. **Map Canvas Cache**: A single off-screen canvas (128x128 pixels) is built from the normalized grids using `ImageData`. This canvas is cached and only rebuilt when:
   - The world tick changes
   - Tiles change (`tilesOk` status changes)
   - World size changes

3. **Viewport Rendering**: Each frame, the renderer:
   - Calculates visible world bounds from camera position and zoom
   - Uses `ctx.drawImage()` to draw the relevant portion of the cached map canvas
   - Scales and transforms the map canvas to match the viewport
   - Applies image smoothing based on zoom level (smooth at low zoom, crisp at high zoom)

4. **Performance**: This approach avoids drawing 16,384 individual rectangles every frame. Instead, it performs a single `drawImage` call per frame, which is GPU-accelerated and very fast.

5. **Fallback**: If tiles are missing or invalid (`tilesOk = false`), the renderer shows a checkerboard pattern with "Waiting for tiles..." message.
...
```json
{
  ...
  "checks": {
    ...
    "tiles": { "ok": true, "message": "Tiles OK: 16384 (expected 16384)" }
  },
...
- Fix: All SQL queries now use `ctx.storage.sql.exec()` pattern
...
9. Map rendering:
   - Full 128x128 world visible on load
   - "Reset View" shows entire world centered
   - Zoom in/out with wheel (no passive listener warnings)
   - Pan by drag works smoothly
   - Debug panel shows tilesCount, expectedTiles, tilesOk, renderMode
